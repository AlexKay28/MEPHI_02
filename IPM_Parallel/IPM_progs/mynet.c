              ...
              int* nl, char* pname, double* tick)
{
  ...
  i = MPI_Init(argc,argv); // получаем указатели на число аргументов программы и на вектор аргуметов соответсвенно 
  if (i != 0){
    fprintf(stderr,"MPI initialization error");
    exit(i);
  }

  MPI_Comm_size(MPI_COMM_WORLD,np); // получить колличество процессов в группе
  MPI_Comm_rank(MPI_COMM_WORLD,mp); // получить ранг процесса в группе
  MPI_Get_processor_name(pname,nl); // получить имя процессора. номер?

  *tick = MPI_Wtick(); // время в секундах между последовательными импульсами
