...
// Time loop:
  ...
  do {
    ...
    if (np2>1) {
      if (mp_l>=0) {
        i2 = 0;
        for (i1=0; i1<nc1; i1++) { m = nc1 * i2 + i1; ss_l[i1] = yy1[m]; }
      }

      if (mp_r>=0) {
        i2 = nc2m;
        for (i1=0; i1<nc1; i1++) { m = nc1 * i2 + i1; ss_r[i1] = yy1[m]; }
      }
    ...
    for (i2=0; i2<nc2; i2++) {
      j2 = i21 + i2;
      if (j2==0) {
        for (i1=0; i1<nc1; i1++) { m = nc1 * i2 + i1; yy2[m] = g11(tv); }
      }
      else if (j2==n2) {
        for (i1=0; i1<nc1; i1++) { m = nc1 * i2 + i1; yy2[m] = g12(tv); }
      }
      else {
        for (i1=0; i1<nc1; i1++) {
          j1 = i11 + i1;
          ...
          if (j1==0) {
            aa[i1] = 0.0; bb[i1] = 0.0; cc[i1] = 1.0; ff[i1] = g21(tv);
          }
          else if (j1==n1) {
            aa[i1] = 0.0; bb[i1] = 0.0; cc[i1] = 1.0; ff[i1] = g22(tv);
          }
          else {
            aa[i1] = aa1[m]; bb[i1] = bb1[m]; cc[i1] = 1.0 + aa[i1] + bb[i1];

            if (i2==0)    s1 = aa1[m] * (yy1[m] - rr_l[i1]);
            else          s1 = aa1[m] * (yy1[m] - yy1[m-1]);

            if (i2==nc2m) s2 = bb1[m] * (rr_r[i1] - yy1[m]);
        ...
        for (i1=0; i1<nc1; i1++) { m = nc1 * i2 + i1; yy2[m] = y1[i1]; }
      }
    }
  ...
  //if (ntv>ntm) printf("ntv>ntm\n");
  } while ((ntv<ntm) && (gt>epst));

  t1 = MPI_Wtime() - t1;

  sprintf(sname,"ex14c_2_%02d.dat",np);
