...
{
  int m, i, i1, i2, j1, j2, i11, i12, i21, i22, nc1, nc2, nc1m, nc2m, nc12;
...
// Time loop:
  ...
  do {
    ...
    for (i1=0; i1<nc1; i1++) {
      j1 = i11 + i1;
      if (j1==0) {
        for (i2=0; i2<nc2; i2++) { m = nc1 * i2 + i1; yy2[m] = g11(tv); }
      }
      else if (j1==n1) {
        for (i2=0; i2<nc2; i2++) { m = nc1 * i2 + i1; yy2[m] = g12(tv); }
      }
      else {
        for (i2=0; i2<nc2; i2++) {
          j2 = i21 + i2;
          ...
          if (j2==0) {
            aa[i2] = 0.0; bb[i2] = 0.0; cc[i2] = 1.0; ff[i2] = g21(tv);
          }
          else if (j2==n2) {
            aa[i2] = 0.0; bb[i2] = 0.0; cc[i2] = 1.0; ff[i2] = g22(tv);
          }
          else {
            aa[i2] = aa2[m]; bb[i2] = bb2[m]; cc[i2] = 1.0 + aa[i2] + bb[i2];

            if (i1==0)    s3 = aa1[m] * (yy1[m] - rr_l[i2]);
            else          s3 = aa1[m] * (yy1[m] - yy1[m-1]);

            if (i1==nc1m) s4 = bb1[m] * (rr_r[i2] - yy1[m]);
            else          s4 = bb1[m] * (yy1[m+1] - yy1[m]);

            s5 = tau05 * f(xx1[i1],xx2[i2],tv);

            ff[i2] = yy1[m] + s4 - s3 + s5;
          }
        }
        ...
        for (i2=0; i2<nc2; i2++) { m = nc1 * i2 + i1; yy2[m] = y1[i2]; }
      }
    }
    ...
    if (lp>0) {
      ...
      for (i2=0; i2<nc2; i2++) {
        ...
        for (i1=0; i1<nc1; i1++) {
          j1 = i11 + i1;
          m = nc1 * i2 + i1;
          fprintf(Fo,"i1=%8d i2=%8d x1=%12le x2=%12le yy=%12le\n",
